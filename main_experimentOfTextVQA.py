'''
1. TextVQA 数据集实验 200个样本
生成 experiment_results 文件保存结果
...
class Config:
    ...
    # 输出路径
    OUTPUT_DIR = "./metadata_1_experiment_results"
    RESULTS_JSON = os.path.join(OUTPUT_DIR, "results1.json")
    STATS_CSV = os.path.join(OUTPUT_DIR, "statistics.csv")
    SAM_SEGMENTS_DIR = os.path.join(OUTPUT_DIR, "sam_segments")  # 添加SAM分割图像保存路径

    # 实验设置,样本数，随机种子
    NUM_SAMPLES = 81  # 自建数据集
    RANDOM_SEED = 42
...
class ExperimentManager:
    def __init__(self, config: Config):
        ...
        os.makedirs(config.SAM_SEGMENTS_DIR, exist_ok=True)  # 添加这一行
